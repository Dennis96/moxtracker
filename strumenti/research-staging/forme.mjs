// Le forme sintetiche di contribution del benchmark di capacita'.
//
// Sono dimensionate sulle misure del corpus reale (G4: contribution media
// 3.332 byte, p95 5.379, massimo 7.983; al massimo 3 game, 63 eventi per game,
// 77 voci di main e 19 copie di sideboard). Il worst-case legale e' il massimo
// che il validator ammette. Stanno qui, e non in `capacita.mjs`, perche' la
// stima delle righe scritte si prova proprio su queste forme.
...
export const GOLDEN = JSON.parse(readFileSync(QUI + "../../prove/fixtures/research-golden-rev2.json", "utf8"));
export const copia = (x) => JSON.parse(JSON.stringify(x));
export const esa = (n) => randomBytes(n).toString("hex");
...
export function contribution(forma) {
