...
export function configResearch(ambiente = {}, adesso = Date.now()) {
  ...
  let motivo = null;
  if (ambiente.RESEARCH_ENABLED !== "true") motivo = "spenta";
  else if (maxContributi === null) motivo = "cap_contributi_non_valido";
  else if (maxQuery === null) motivo = "budget_d1_non_valido";
  else if (deployment === null) motivo = "deployment_non_valido";
  else if (!chiaviValide) motivo = "chiavi_non_valide";
...
// Da spenta i cap non si pubblicano: il client non deve comporre richieste
// su numeri che nessuno ha qualificato.
